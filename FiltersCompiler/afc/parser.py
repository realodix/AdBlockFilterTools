...
def parse_line(line, position='body'):
    ...
    position="body", the parser only recognizes filters, comments, processing
    instructions and empty lines. If position="metadata", it in addition recognizes
    metadata. If position="start", it also recognizes headers.

    Note: Checksum metadata lines are recognized in all positions for backwards
    compatibility. Historically, checksums can occur at the bottom of the filter
    list. They are are no longer used by Adblock Plus, but in order to strip them
    (in afc.renderer), we have to make sure to still parse them regardless of their
